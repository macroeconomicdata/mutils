...
%    x = x13(..., '-s -w'),
% or x = x13(..., 'noflags')
% If no flag is set by the user, the default is to set the '-s -n' flags,
% so that the diagnostics file is generated (-s) and only the requested
% tables (-n) are written to the .out property. The 'noflags' option
% removes all the flags, including the default. 
%
...
% is the name of the executable. By default, this is 'x13as', or 'x12a' (or
% 'x12a64' on a 64-bit computer) if the 'x-12' option is set. But it is
% also possible to specify a m-file that perfoms the necessary seasonal
        ...
        if exist(prog,'file')
            ...
            if useX12
                ...
                if success
                    ...
                    if useHTML
                        theZIP  = 'x13ashtml_V1.1_B39.zip';
                    else
                        theZIP  = 'x13as_V1.1_B39.zip';
                    end
                    err = MException('X13TBX:x13:ProgramMissing', ...
