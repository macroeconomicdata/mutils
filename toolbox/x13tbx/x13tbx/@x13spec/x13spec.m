...
% does not use for the 'period' and 'options' sections (but your own m-file
% could use these sections).
%
% -------------------------------------------------------------------------
...
% internally, and called by makespec.m, x13series.m, or x13.m, but they
% could also be used by a user:
%
...
%             end
    ...
    % --- set transform-function from transform-power if possible ---------
        ...
        function obj = Set_TransformFunction_From_TransformPower(obj)
            ...
            if isempty(TP)      % power not specified, nothing to do
                return;
            elseif isempty(TF)  % power specifiedm but function not
    ...
    % --- remove inconsistencies and enforce compatibility ----------------
        ...
        % particular seasonal adjustment program. 'prog' can be 'x13as.exe',
        % for example.
            switch prog
                case {'x13as.exe','x13ashtml.exe'}
                    obj = obj.enforceX13;
                case {'x12s.exe','x12a64.exe'}
                    obj = obj.enforceX12;
                case 'x11.m'
                    obj = obj.enforceX11;
                case 'method1.m'
