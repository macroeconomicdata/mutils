...
classdef ( InferiorClasses = { ...
    ...
    properties(Constant)
        version = '1.50';       % version of the toolbox
    end
    ...
    properties
        ...
        keyv         = struct('dat','dat','tr','tr','sa','sa','sf','sf', ...
            'ir','ir','si','si','rsd','rsd');	% key variables: data, trend, ...
                                                % seasonally adjusted, etc.
    end
...
%         end
        ...
        % value (useful to terermine if a log-regression needs to be
        % applied)
...
%            p = x13spec.toNum(p);
        ...
        % into the x13series object
            ...
            if (adjustTD || adjustEASTER)
                ...
                % If we are only interested in the coefficients and not in the quality
                % measures, a quicker way to do this is with mldivide:
                % beta = [ones(nobs,1),exo] \ data.data;
                % rsd = table2array(mdl.Residuals(:,1));  % extract residuals

                d18 = exo * beta(2:end);    % variation of irregular explained by
