...
selected = {'gdp constant prices', 'personal consumption expenditures',...
            'industrial production', 'initial jobless claims'};
...
% file. We'll need to tell the function whether to detrend data or
% standardize data. Aditionally, we want to aggregate initial jobless
% claims to monthly since UMIDAS() is for monthly-quarterly data. To do so,
% we can set the 'frequency' of initial jobless claims to monthly in the
% table 'lib':
lib(strcmp(lib.series_name, 'initial jobless claims'),4) = {'monthly'};
rm_trend = true;
standardize = true;
